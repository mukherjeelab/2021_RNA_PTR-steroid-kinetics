...
# g <- c("MSI1","MSI2")
# myTxi_exvivo$abundance$matureGene %>%
#   filter(symbol %in% g) %>%
...
#   filter(Tx=="AngII") %>%
#   ggpubr::ggbarplot(data = ., x = "Time", y = "TPM", facet.by = "symbol", merge = T, add = c("jitter","mean")) %>% ggsave(filename = "~/Desktop/MSI1_MSI2_exvivo.pdf", plot = .)
# 
# bind_rows(
# myTxi_exvivo$abundance$matureGene %>%
#   filter(symbol %in% g) %>%
...
#   separate(data = ., col = sample, into = c("Tx","Time","Rep"), sep = "_", remove = T) %>%
#   mutate(type="H295R")
# ) %>% write_csv(x = ., path = "~/Desktop/MSI1_MSI2_exvivo_H295R.csv.gz")
#   
#   ggplot(data = ., aes(x=as.numeric(Time), y=TPM, color=Tx)) +
#   geom_point() +
#   ggtitle(label = g) +
#   theme_few() +
#   facet_grid(~symbol)
...
rownames(qcCor) <- colnames(qcCor)
  ...
  nCut <- 8
pheatmap::pheatmap(qcCor, clustering_distance_rows = "euclidean",clustering_distance_cols = "euclidean", clustering_method = "ward.D2", cutree_cols = nCut, cutree_rows = nCut, color = viridis(10, option = "E", direction = -1), border_color = "black", treeheight_row = 15, treeheight_col = 15, fontsize = 10, width = 8, height = 8, filename = here("fig1_exvivo","plots","qcHeatmap_exvivo.pdf"), silent = T)
dev.off()
...
myLC <- levels(pca_plot_data$condition)[c(13,15,11,12,14,3,5,1,2,4,8,10,6,7,9)]
...
myCols <- c(RColorBrewer::brewer.pal(n = 6, name = "Greys")[2:6],
            RColorBrewer::brewer.pal(n = 6, name = "Reds")[2:6],
            RColorBrewer::brewer.pal(n = 6, name = "Blues")[2:6])
...
p_pca_mean <- ggplot(pca_plot_data_mean, aes(x=-PC2, y=-PC1, color = tx, label=time)) +
  geom_point(size=2) + 
