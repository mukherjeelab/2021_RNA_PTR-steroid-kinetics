...
ggsave(here("fig4_PTR-H295R", "plots", paste0("exon_intron_", targ, "_sem_spliteisa.pdf")), g, width = 10, height = 8)
```
...
split_exon_intron_plot <- function(target, intron_temp, exon_temp, df) {
  ...
  ggplot(temp, aes(x = time, y = expr, color = eisa)) +
    facet_grid(.~type) +
    geom_line() +
    geom_errorbar(aes(x = time, ymin = expr - sem, ymax = expr + sem), width = 0.2) +
    geom_text(data = temp %>% filter(time == 24) %>% select(-c(time)) %>% distinct(), 
...
ggsave(here("fig4_PTR-H295R", "plots", paste0("exon_intron_", targ, "_sem_splitintronexon.pdf")), g, width = 10, height = 8)
```
...
getvalues <- function(gene) {
  exon1 <- exon_temp %>% filter(symbol == gene) %>% mutate(cat = "exon", lfc = expr)
  intron1 <- intron_temp %>% filter(symbol == gene) %>% mutate(cat = "intron", lfc = expr)
  bind_rows(exon1, intron1)
}
...
ps <- sapply(simplify = F, 
       c("STAR", "CYP11A1", "HSD3B2", "CYP21A2", "CYP11B1", "CYP11B2"), 
       function(x) {
         ggpubr::ggline(data = getvalues(x), 
                        x = "time", y = "lfc", group = "cat", 
                        numeric.x.axis = T, add = "mean", 
                        error.plot = "errorbar", color = "cat", 
                        palette = c("red","black"), 
                        title = paste0(x, ": ", corClusters %>% filter(symbol == x) %>% pull(cor) %>% round(digits = 3))) + ggpubr::rremove("y.text")})

g <- cowplot::plot_grid(plotlist = ps, ncol = 1)

ggsave(here("fig4_PTR-H295R", "plots", "pathwaygenes_noylab.pdf"), g, width = 3, height = 15)
```
