...
```{r}
featurescan <- function(anno, allseqs, ctrlseqs, 
                             clustername, eisadir = "down",
                             PWM_list = CISBPRNA_hs_PWM) {
  case_genes <- anno %>% filter(cluster %in% clustername & eisa == eisadir) %>% pull(symbol)
  case <- allseqs %>% filter(symbol %in% case_genes) %>% pull(seq) %>% DNAStringSet()
  ctrl <- DNAStringSet(ctrlseqs$seq)
  CISBP_compare <- motif_compare(CISBPRNA_hs_PWM, case, ctrl)
  RBNS_compare <- motif_compare(RBNS_PWM, case, ctrl)
  list(CISBP_compare = CISBP_compare, RBNS_compare = RBNS_compare)
}

featureplot <- function(scanned, pcutoff = 0.01, motifs = "RBNS") {
  if (motifs == "RBNS") {
    stringreplace <- "_.*"
  } else {
    stringreplace <- "*.+_"
  }
  g1 <- ggplot(scanned, aes(x = log2FC, y = -log10(p_adj))) +
    geom_point(color = ifelse(scanned$p_adj < pcutoff, "red", "black")) +
    theme_classic() +
    geom_text_repel(aes(label=ifelse(p_adj < pcutoff,
                                   gsub(stringreplace,"",motif),
                                   "")))
  g1
}

# res <- featurescan(anno, allseqs, ctrlseqs, "aw3")
res <- sapply(c("as3", "as4", "aw2", "aw3"), function(x) featurescan(anno, allseqs, ctrlseqs, x),
       simplify = FALSE)

saveRDS(res, here("fig4_PTR-H295R","tmp","motifcomps.rds"))

y <- "aw3"
g <- sapply(names(res[[y]]), function(x) featureplot(res[[y]][[x]], motifs = str_remove(x, "_compare")),
       simplify = FALSE)

ggsave(filename = here("fig4_PTR-H295R","plots","motifcompare_aw3_CISBP.pdf"), plot = g[[1]], width = 4, height = 4)
ggsave(filename = here("fig4_PTR-H295R","plots","motifcompare_aw3_RBNS.pdf"), plot = g[[2]], width = 4, height = 4)

resdf <- sapply(names(res), 
       function(x) do.call(rbind, res[[x]]) %>% rownames_to_column("comp") %>% mutate(comp = str_remove(comp, "\\..+")),
       simplify = FALSE) %>% 
  do.call(rbind, .) %>% rownames_to_column("cluster") %>% mutate(cluster = str_remove(cluster, "\\..+")) %>% 
  mutate(motif = ifelse(comp == "RBNS_compare", 
                        str_remove(motif, "_.*"),
                        str_remove(motif, ".+_")))

resR <- resdf %>% filter(comp == "RBNS_compare")
g2 <- ggplot(resR, aes(x = log2FC, y = -log10(p_adj))) +
  facet_wrap(.~cluster) +
  geom_point(color = ifelse(resR$p_adj < 0.01, "red", "black")) +
  theme_classic() +
  geom_text_repel(aes(label=ifelse(p_adj < 0.01, motif,"")))

resC <- resdf %>% filter(comp == "CISBP_compare")
g1 <- ggplot(resC, aes(x = log2FC, y = -log10(p_adj))) +
  facet_wrap(.~cluster) +
  geom_point(color = ifelse(resC$p_adj < 0.01, "red", "black")) +
  theme_classic() +
  geom_text_repel(aes(label=ifelse(p_adj < 0.01, motif,"")))

ggsave(filename = here("fig4_PTR-H295R","plots","motifcompare_4clusters_CISBP.pdf"), plot = g1, width = 8, height = 8)
ggsave(filename = here("fig4_PTR-H295R","plots","motifcompare_4cluster_RBNS.pdf"), plot = g2, width = 8, height = 8)
